...
\twocolumn[
    ...
    \title{VRにおけるブレス攻撃時の表示方法による視認性とゲーム体験の比較}
...
その結果、攻撃エフェクトによってターゲットがどのような行動を起こすかが確認できなく、視認性と操作性の低下につながります。
...
本研究では，ブレス攻撃エフェクトによる視界遮蔽が視認性およびゲーム体験に与える影響を調査する．プレイヤーはブレス攻撃をランダム移動するターゲットに対して発射し，各表示条件を比較する． 
...
\begin{figure}[htbp]
\begin{center} 
\includegraphics[width=1\linewidth]{fig/fire.png} 
\caption{各表示条件の比較} 
\label{fig:act7}
\end{center}
\end{figure}
...
一分間でスコアを競ってもらう．
敵はランダムな時間で赤色と青色に切り替わり,青色は攻撃可能状態,
赤色は攻撃不可状態を表しており(\figref{fig:act3}),
攻撃不可状態で攻撃した場合はスコアを減点します.
また，ブレス攻撃はチャージ制とし，
...
\begin{figure}[htbp] 
\begin{center} 
\includegraphics[width=0.8\linewidth]{fig/ghostdif.png} 
\caption{左 攻撃可能状態, 右 攻撃不可状態} 
\label{fig:act3} 
\end{center} 
\end{figure}
...
\begin{figure}[htbp] 
\begin{center} 
\includegraphics[width=0.8\linewidth]{fig/act8.png} 
\caption{追従誤差の説明} 
\label{fig:act8} 
\end{center} 
\end{figure}
